...
function routeSirpc(action, payload) {
  switch (action) {
    case 'ping': return { message: 'SIRPC activo', timestamp: nowText(), version: 'V2 Integral' };
...
function listarRevisiones(payload) {
  ...
  revs.forEach(r => {
    ...
    if (tipo && String(row.TipoRevision) !== tipo) return;
  ...
  out.sort((a,b) => String(a.FechaCita + a.HoraCita + a.TipoRevision).localeCompare(String(b.FechaCita + b.HoraCita + b.TipoRevision)));
  return { revisiones: out, total: out.length };
}
